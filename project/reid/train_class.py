...
from utils.transforms import Random2DTranslation
from utils.utils import Logger, AverageMeter, save_checkpoint
from utils.losses import CrossEntropyLabelSmooth, DeepSupervision
from utils.optimizers import init_optim
from utils.eval_metrics import evaluate

parser = argparse.ArgumentParser(description='Train image model with cross entropy loss')
# Datasets
parser.add_argument('--root', type=str, default=r"E:\files\programming\dataset\reid", help="root path to data directory")
parser.add_argument('-d', '--dataset', type=str, default='market1501',
                    choices=datamanager.get_names())
...
parser.add_argument('--stepsize', default=60, type=int, help="stepsize to decay learning rate (>0 means this is enabled)")
...
def main():
    ...
    dataset = datamanager.init_img_dataset(
    ...
    trainloader = DataLoader(
        ...
        batch_size=args.train_batch, num_workers=args.workers, shuffle=True,
    ...
    if args.evaluate:
        print("Evaluate only")
        vali(model, queryloader, galleryloader, use_gpu)
        return
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        if (epoch + 1) > args.start_eval and args.eval_step > 0 and (epoch + 1) % args.eval_step == 0 or (
            ...
            print("==> Test")
            rank1 = vali(model, queryloader, galleryloader, use_gpu)
...
def vali(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20]):
    batch_time = AverageMeter()
